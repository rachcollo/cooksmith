...
describe('weekly preparation Edge Function contracts', () => {
  ...
  it('returns privacy-safe household flow failure categories', () => {
    ...
    expect(source).toContain('const hasUnreadyRecipes =')
  ...
  it('requires current v2 enrichment and never returns a deterministic fallback checklist', () => {
    ...
    expect(householdSource).toContain('const usableEnrichments = enrichments.filter(')
    expect(householdSource).toContain('if (hasUnreadyRecipes)')
    expect(householdSource).toContain('if (usableEnrichments.length === 0)')
