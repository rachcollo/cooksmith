...
describe("recipe library experience", () => {
  it("loads an empty library and creates a recipe that opens in detail", async () => {
    ...
    await userEvent.click(
      within(createDialog).getByRole("button", { name: "Save recipe" }),
    );
  ...
  it("searches, clears no-result state, edits, cancels and archives recipes", async () => {
    ...
    expect(
      (await screen.findAllByRole("heading", { name: "Lentil soup" }))[0],
    ).toBeVisible();
    ...
    expect(
      screen.getByRole("heading", { name: "Apple crumble" }),
    ).toBeVisible();

    await user.click(
      screen.getAllByRole("button", { name: "Open details" })[1],
    );
    ...
    await user.type(
      within(secondDialog).getByLabelText("Recipe name"),
      "Apple crumble tray",
    );
    await user.click(
      within(secondDialog).getByRole("button", { name: "Save changes" }),
    );
    ...
    expect(
      (
        await screen.findAllByRole("heading", { name: "Apple crumble tray" })
      )[0],
    ).toBeVisible();
    ...
    expect(
      screen.queryByRole("heading", { name: "Apple crumble tray" }),
  ...
  it("shows validation and repository failures, then refreshes for household changes", async () => {
    const list = vi.fn(async () => [
      recipe({}),
    ]) satisfies RecipeRepository["list"];
    const repository: RecipeRepository = {
      list,
      create: async () => {
