...
function BootstrapHarness({ client }: { client: CooksmithSupabaseClient | null }) {
  ...
  if (error) return <AuthCallbackError error={error} />
...
describe('deterministic authentication bootstrap', () => {
  it('shows a safe callback failure instead of redirecting when PKCE exchange fails', async () => {
    const exchangeError = new Error('provider details are not shown')
    ...
    expect(
      await screen.findByRole('heading', { name: 'That sign-in link could not be completed' }),
    ).toBeInTheDocument()
    expect(screen.getByText(/Please request a new magic link/)).toBeInTheDocument()
  ...
  it('shows a safe callback failure when PKCE exchange returns no session', async () => {
    const client = clientWithAuth({
    ...
    window.history.replaceState(null, '', '/?code=empty-session-code')
    await expect(bootstrapAuth(client)).rejects.toMatchObject({
      category: 'pkce_exchange_empty',
    })
    expect(window.location.search).toBe('')
  })

  it('uses a valid exchanged PKCE session even if immediate getUser would fail', async () => {
    const user = { id: 'hosted-callback-user' } as User
    const session = { user } as Session
    const exchangeCodeForSession = vi.fn(async () => ({ data: { session, user }, error: null }))
    const getUser = vi.fn(async () => ({
      data: { user: null },
      error: new Error('persistence race'),
    }))
    const signOut = vi.fn(async () => ({ error: null }))
    const client = clientWithAuth({ exchangeCodeForSession, getUser, signOut })

    window.history.replaceState(null, '', '/?code=valid-hosted-code')
    render(<BootstrapHarness client={client} />)

    await screen.findByRole('heading', { name: 'Dinner decisions, made lighter.' })
    expect(exchangeCodeForSession).toHaveBeenCalledOnce()
    expect(exchangeCodeForSession).toHaveBeenCalledWith('valid-hosted-code')
  ...
  it('rejects a PKCE exchange that returns a session without a user', async () => {
    ...
    window.history.replaceState(null, '', '/?code=no-user-code')
    try {
      await bootstrapAuth(client)
      throw new Error('Expected bootstrapAuth to fail')
    } catch (error) {
      expect(error).toBeInstanceOf(AuthBootstrapError)
      expect(error).toMatchObject({ category: 'pkce_validation_failed' })
    }
  })
