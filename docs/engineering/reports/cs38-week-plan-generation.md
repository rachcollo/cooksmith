...
Recipes and Plan now expose one shared **Plan my week** journey. It deterministically proposes visible recipes for empty dinner days, preserves occupied days, explains recipe shortfalls, supports per-day replacement or removal before Apply, and requires a separate confirmation before a complete week can be replaced.
...
- `npm ci`, format/check, documentation command audit, lint, type checking, 41 Vitest files (188 tests), production build and 12 Playwright desktop/mobile shell checks pass.
