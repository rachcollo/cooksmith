...
import {
  useFeatureFlagRepository,
  useFeatureFlags,
} from "../app/admin/featureFlagContext";
  ...
  return (
    <Stack gap="large">
      ...
      <Panel>
        ...
        <p>
          Review current recipe insight status, retry genuine failures and edit
          household recipes.
        </p>
        <Link
...
function RecipeEnrichmentOperations() {
  ...
  const [status, setStatus] = useState<RecipeEnrichmentBackfillStatus | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    void repository
      ...
      .catch(() => {
        if (sequence !== refreshSequence.current) return;
        setMessage(
          "Recipe enrichment progress could not be loaded. Try again.",
        );
      });
    return () => {
  ...
  useEffect(() => {
    ...
    const hasActiveWork =
      (status.states.pending ?? 0) > 0 || (status.states.processing ?? 0) > 0;
  ...
  async function setAi(enabled: boolean) {
    ...
    try {
      ...
      setMessage(
        "Recipe Intelligence AI could not be updated. No setting was changed.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section className="flow-stack" aria-labelledby="recipe-enrichment-title">
      <div>
        ...
        <p>
          Prepare household and shared recipes for Get Ahead without changing
          recipe content.
        </p>
      </div>
        ...
        {status ? (
          <>
            ...
            <div className="cluster">
              <Button
                disabled={busy}
                onClick={() => void setAi(!status.aiEnabled)}
              >
                {status.aiEnabled
              ...
              <Button
                variant="secondary"
                disabled={
                  busy || status.recoverableCount === 0 || !status.aiEnabled
                }
...
function WeeklyPreparationOperations() {
  const repository = useWeeklyPreparationAdminRepository();
  const [settings, setSettings] = useState<WeeklyPreparationSettings | null>(
    null,
  );
  const [evaluation, setEvaluation] =
    useState<WeeklyPreparationEvaluation | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch(() => {
        if (active)
          setMessage("Weekly preparation controls could not be loaded.");
      })
      .finally(() => {
  ...
  if (!repository) return null;
  if (loading)
    return <LoadingState label="Loading weekly preparation operations" />;
  if (!settings)
    return (
      <FeedbackState
        tone="info"
  ...
  async function runEvaluation() {
    ...
    try {
      ...
      await refresh();
      setMessage(
        "The 30-plan evaluation finished. Review the evidence before accepting it.",
      );
    } catch (error) {
      setMessage(
  ...
  async function acceptEvaluation() {
    if (
      !evaluation ||
      !window.confirm("Accept this exact completed evaluation?")
    )
      return;
    setEvaluating(true);
    setMessage("");
    try {
      ...
      await refresh();
      setMessage(
        "Evaluation accepted. AI assistance still requires a separate confirmation.",
      );
    } catch {
      setMessage(
        "This evaluation cannot be accepted. Review failed or incomplete cases first.",
      );
    } finally {
      setEvaluating(false);
    }
  ...
  async function update(
    next: Pick<WeeklyPreparationSettings, "aiEnabled" | "emergencyStop">,
  ) {
    ...
    try {
      ...
      setMessage(
        "Weekly preparation controls updated and recorded in the audit log.",
      );
    } catch {
      setMessage(
        next.aiEnabled
  ...
  return (
    <section
      ...
      aria-labelledby="weekly-preparation-operations-title"
    >
      <div>
        <h2 id="weekly-preparation-operations-title">
          Weekly preparation operations
        </h2>
        <p>
          Prove readiness, review evidence and activate model assistance in
          separate steps.
        </p>
      </div>
      <p aria-live="polite">{message}</p>
      <Panel>
        ...
        <p>
          The 30-plan evaluation means 30 example weekly plans. It is not a
          30-minute Get Ahead session.
        </p>
        <ol>
          <li>
            Configuration:{" "}
            {settings.modelIdentifier ? "Ready" : "Needs attention"}
          </li>
          <li>
            Hosted smoke test:{" "}
            {settings.smokeVerified ? "Ready" : "Not verified"}
          </li>
          <li>
            30-plan evaluation:{" "}
            {evaluation?.status === "completed" ? "Completed" : "Not completed"}
          </li>
          <li>
        ...
        <details className="evaluation-case">
          ...
          <div className="flow-stack">
            <p>
              These protected rules are applied automatically. You do not need
              to review recipes or correct individual evaluation cases.
            </p>
            <ul>
              <li>Sessions must stay within 15, 30 or 60 minutes.</li>
              <li>
                Tasks must use supplied recipe preparation and take at least 5
                minutes.
              </li>
              <li>Raw-protein and cross-contamination work is excluded.</li>
              <li>
                Preheating, serving steps and reserving water are not advance
                preparation.
              </li>
              <li>
                Cooksmith must prefer useful midweek time savings and return no
                tasks when appropriate.
              </li>
            </ul>
            <p>
              Safety, traceability and time-budget rules are versioned and
        ...
        {!settings.aiEnabled &&
        (!settings.smokeVerified || !evaluation?.accepted) ? (
          <p>
            Enable AI assistance becomes available after the hosted smoke test
            and the current evaluation are accepted.
          </p>
        ) : null}
      </Panel>
      <Panel>
        ...
        {evaluation ? (
          <>
            ...
            {evaluation.failedCases.length > 0 ? (
              <div
                className="evaluation-case-review"
                aria-labelledby="failed-case-review-title"
              >
                <div>
                  <h4 id="failed-case-review-title">
                    Automatic quality findings
                  </h4>
                  <p>
                    Cooksmith uses these technical details to improve the
                    planner centrally. No recipe-by-recipe action is required
                    from you.
                  </p>
                </div>
                <details className="evaluation-case">
                  ...
                  {evaluation.failedCases.map((evaluationCase) => {
                    const guidance = evaluationReviewGuidance(
                      evaluationCase.reason,
                    );
                    return (
                      <details
                        key={evaluationCase.caseNumber}
                      ...
                      >
                        <summary>
                          ...
                          <span>
                            {evaluationReasonLabel(evaluationCase.reason)}
                          </span>
                        </summary>
                        <div className="flow-stack">
                          <dl className="admin-metrics-grid">
                          ...
                          <div>
                            ...
                            {evaluationCase.generatedTasks.length > 0 ? (
                              <ol>
                                {evaluationCase.generatedTasks.map(
                                  (task, index) => (
                                    <li key={`${task.title}-${index}`}>
                                      <strong>{task.title}</strong> (
                                      {task.estimatedMinutes} min, saves{" "}
                                      {task.estimatedTimeSavedMinutes} min
                                      midweek)
                                    </li>
                                  ),
                                )}
                              ...
                              <p>
                                No tasks were returned, or task evidence was not
                                recorded for this earlier run.
                              </p>
                            )}
                          </div>
                        </div>
            ...
            <dl className="admin-metrics-grid">
              ...
              <Metric
                label="Deterministic"
                value={evaluation.deterministicCount}
              />
              <Metric label="Model calls" value={evaluation.modelCallCount} />
              <Metric
                label="Valid outputs"
                value={evaluation.validOutputCount}
              />
              <Metric
                label="Reviewed correct"
                value={evaluation.reviewedCorrectCount}
              />
              <Metric
                label="Unsupported data"
                value={evaluation.unsupportedCount}
              />
              <Metric label="Fallbacks" value={evaluation.fallbackCount} />
              <Metric
                label="Quality failures"
                value={evaluation.qualityFailureCount}
              />
              <Metric
                label="Average latency"
                value={`${evaluation.averageLatencyMs} ms`}
              />
              <Metric
                label="Token usage"
              ...
              />
              <Metric
                label="Chop/dice decision"
                value={evaluation.ambiguousDecision}
              />
            </dl>
          </>
        ) : (
          <p>
            No hosted evaluation has been recorded. AI assistance must remain
            disabled.
          </p>
        )}
      </Panel>
...
function evaluationReviewGuidance(reason: string): {
  area: string;
  explanation: string;
} {
  switch (reason) {
    ...
    default:
      return {
        ...
        explanation:
          "The generated plan did not satisfy Cooksmith’s release-quality rules.",
      };
  }
}
