...
-- Deterministic enrichment is the safe baseline and must not suppress recovery of a
-- failed provider-assisted job for the same current recipe version.
...
comment on function cooksmith_private.recipe_enrichment_recoverable_count() is
  'Counts exhausted current-version provider jobs unless a provider-assisted enrichment is already active.';
comment on function cooksmith.recipe_enrichment_backfill_command(text, integer) is
  'Admin-only audited enrichment control with provider-specific recovery selection.';
