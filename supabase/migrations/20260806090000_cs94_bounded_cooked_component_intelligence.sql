...
alter table cooksmith.recipe_enrichment_jobs
  alter column schema_version set default 'recipe-intelligence-v3',
  alter column rules_version set default 'cooksmith-rules-v3';
...
select
  ...
  versions.id, 'recipe-intelligence-v3', 'cooksmith-rules-v3', 'provider-assisted-v1'
...
set ai_enabled = false,
  corpus_version = 'weekly-preparation-corpus-v13',
  prompt_version = 'weekly-preparation-strategy-v13',
...
where planner_version <> 'weekly-preparation-planner-v13';
  ...
  if new.ai_enabled and not new.emergency_stop and (
    ...
    or not exists (
      ...
      where run.status = 'completed'
        ...
        and acceptance.planner_version = 'weekly-preparation-planner-v13'
    ...
    raise exception 'Current v13 smoke test, recipe coverage and accepted evaluation required'
      using errcode = '23514';
  end if;
...
begin
  select pg_get_functiondef('cooksmith.admin_recipe_enrichment_list(text,text)'::regprocedure)
    into definition;
  execute replace(replace(definition,
    'recipe-intelligence-v2', 'recipe-intelligence-v3'),
    'cooksmith-rules-v2', 'cooksmith-rules-v3');
end;
$$;

comment on function cooksmith_private.weekly_preparation_recipes_ready() is
  'Release gate requiring bounded v3 stages for every current recipe and no unfinished v3 jobs.';
comment on function cooksmith.audit_weekly_preparation_settings() is
  'Audits AI controls and prevents v13 activation until bounded-stage recipe coverage, evaluation and hosted smoke evidence are accepted.';
