...
select
  enrichment.source_kind,
  enrichment.recipe_id,
  enrichment.imported_recipe_id,
  enrichment.household_id,
  enrichment.recipe_version_id,
  enrichment.schema_version,
  enrichment.rules_version,
  'provider-assisted-v2'
from cooksmith.recipe_enrichments enrichment
where enrichment.is_active
  and enrichment.schema_version = 'recipe-intelligence-v2'
  and enrichment.rules_version = 'cooksmith-rules-v2'
  and jsonb_typeof(enrichment.result -> 'preparationOpportunities') = 'array'
  and jsonb_array_length(enrichment.result -> 'preparationOpportunities') = 0
...
set
  ai_enabled = false,
  corpus_version = 'weekly-preparation-corpus-v9',
  prompt_version = 'weekly-preparation-strategy-v9',
...
where planner_version <> 'weekly-preparation-planner-v9';
  ...
  if new.ai_enabled and not new.emergency_stop and (
    ...
    or not exists (
      ...
      where run.status = 'completed'
        ...
        and acceptance.planner_version = 'weekly-preparation-planner-v9'
...
comment on function cooksmith_private.weekly_preparation_recipes_ready() is
  'Release gate requiring every current recipe to have at least one active v2 preparation opportunity and no unfinished v2 jobs.';
comment on function cooksmith.audit_weekly_preparation_settings() is
  'Audits AI controls and prevents activation until real recipe opportunity coverage and current evaluation evidence are complete.';
