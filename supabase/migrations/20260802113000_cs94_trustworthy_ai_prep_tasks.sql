...
set
  ai_enabled = false,
  corpus_version = 'weekly-preparation-corpus-v6',
  prompt_version = 'weekly-preparation-strategy-v6',
...
begin
  ...
  if new.ai_enabled and not new.emergency_stop and not exists (
    ...
    where run.status = 'completed'
      ...
      and acceptance.planner_version = 'weekly-preparation-planner-v6'
...
comment on function cooksmith.audit_weekly_preparation_settings() is
  'Audits AI controls and binds activation to the accepted trustworthy-task evaluation and deployment.';
